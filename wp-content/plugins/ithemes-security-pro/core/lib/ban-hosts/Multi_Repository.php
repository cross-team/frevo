...
final class Multi_Repository implements Source {
	...
	public function get_creation_schema( $source ) {
		...
		if ( ! $repository instanceof Creatable ) {
			throw new Unsupported_Operation( sprintf( esc_html__( 'The ban source "%s" does not support creating a ban.', 'it-l10n-ithemes-security-pro' ), $source ) );
		}
	...
	public function get_update_schema( $source ) {
		...
		if ( ! $repository instanceof Updatable ) {
			throw new Unsupported_Operation( sprintf( esc_html__( 'The ban source "%s" does not support updating a ban.', 'it-l10n-ithemes-security-pro' ), $source ) );
		}
	...
	public function fill( $source, array $data, Repository_Ban $ban = null ) {
		...
		if ( ! $repository instanceof Persistable ) {
			throw new Unsupported_Operation( sprintf( esc_html__( 'The ban source "%s" does not support saving bans.', 'it-l10n-ithemes-security-pro' ), $source ) );
		}

		if ( $ban && $ban->get_source() !== $source ) {
			throw new Invalid_Argument_Exception( sprintf( esc_html__( 'The source "%1$s" does not match the previous ban source "%2$s".', 'it-l10n-ithemes-security-pro' ), $source, $ban->get_source() ) );
		}
	...
	public function persist( Repository_Ban $ban ) {
		...
		if ( ! $repository instanceof Persistable ) {
			throw new Unsupported_Operation( sprintf( esc_html__( 'The ban source "%s" does not support saving bans.', 'it-l10n-ithemes-security-pro' ), $ban->get_source() ) );
		}
	...
	public function delete( Repository_Ban $ban ) {
		...
		if ( ! $repository instanceof Deletable ) {
			throw new Unsupported_Operation( sprintf( esc_html__( 'The ban source "%s" does not support deleting a ban.', 'it-l10n-ithemes-security-pro' ), $ban->get_source() ) );
		}
